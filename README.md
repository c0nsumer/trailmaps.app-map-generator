...
| `url_hash` | No | `true` | Write `#zoom/lat/lon` to the URL hash as the user pans/zooms, and honour any hash on page load. Enables shareable deep-links and reload-preserved position. Set `false` to drop the hash entirely — return visitors always open framed on `bbox`, URLs stay clean, and the last-viewed location doesn't leak via address bar / screenshots / screen-shares. See [Privacy](#privacy) for the trade-off. |
...
**URL hash (`url_hash`, default `true`).** When enabled, the URL gains a `#zoom/lat/lon` fragment that updates live as the user pans/zooms. That fragment never leaves the browser via network traffic (URL fragments are not sent to the server), but it *is* visible in the address bar, included in screenshots, visible during screen-shares, and captured by anyone the user sends the URL to. On a trail map this usually isn't sensitive — riders share "look at this section" links routinely — but a user who was just zoomed in on their home address when they share the map URL is implicitly sharing that location too. For privacy-conscious deployments, set `url_hash: false` in the config and the hash is never written; return visitors always land on the default bbox-framed view, and no shareable deep-links exist.
