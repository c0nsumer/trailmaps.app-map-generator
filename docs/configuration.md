...
| `lane_renderer` | No | `"native"` | How routes that share a path are drawn as parallel lanes. `"native"` expands shared corridors at build time and draws them with MapLibre line layers. `"plugin"` ships maplibre-gl-lanes and lets the browser order and draw the lanes at load time, with crossing-minimized lane order and curved junctions. Under the plugin, route-name labels and the trail highlights ride the lanes too, and the build-time expansion is skipped. One-way chevrons run down the center of the bundle, because one-way is a fact about the trail and not about one route. The plugin path is still under evaluation. Keep the default for production maps. |
