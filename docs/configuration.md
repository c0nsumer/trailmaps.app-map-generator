...
| `lane_renderer` | No | `"plugin"` | How routes that share a path are drawn as parallel lanes. `"plugin"` ships maplibre-gl-lanes, and the browser orders and draws the lanes at load time, with crossing-minimized lane order and curved junctions. Route-name labels and the trail highlights ride the lanes, and the build-time expansion is skipped. One-way chevrons run down the center of the bundle, because one-way is a fact about the trail and not about one route. The plugin needs WebGL2. On a device without WebGL2, the map falls back to the native renderer by itself. `"native"` expands shared corridors at build time and draws them with MapLibre line layers. Native was the default before 2026-09-20. It stays available for now so that one map can be switched back with this key, and it will be removed in a later release. |
