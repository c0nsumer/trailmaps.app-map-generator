...
| `suppress_basemap_drawn_ways` | No | `false` | Hide the Protomaps basemap's own line, casing and name label for every OSM way this map draws a route over. Use it when the basemap's line shows through a dashed route or beside a route that does not sit exactly on it. Paths the map does not draw are left alone, so side trails still provide context. Matching is by OSM way id, so it does not apply to routes whose geometry comes from a local `osm_file` with synthetic ids, or to custom routes defined in the config. Custom base layers unaffected. |
...
| `relation_colors` | No | `{}` | Map of relation ID to CSS color (hex, named, `rgb()`, `rgba()`, `hsl()`); overrides the OSM `colour` tag. Only takes effect on trail polylines when `color_by: relation` (the default). Under `color_by: trail` the override is used only for the route swatch in the finder. |
...
| `lane_renderer` | No | `"native"` | How routes that share a path are drawn as parallel lanes. `"native"` expands shared corridors at build time and draws them with MapLibre line layers. `"plugin"` ships maplibre-gl-lanes and lets the browser order and draw the lanes at load time, with crossing-minimized lane order and curved junctions. Under the plugin, one-way chevrons, route-name labels and the trail highlights ride the lanes too, and the build-time expansion is skipped. The plugin path is still under evaluation. Keep the default for production maps. |
...
- **Credits** section: a "Generated by trailmaps.app Map Generator." line
  ...
  SIL Open Font License always appear; Mapterhorn and maplibre-contour when
  terrain is enabled; USGS 3DEP when route elevation is computed and shown.
...
dark-mode auto-invert, matching `invert_logo_dark` on the primary.
