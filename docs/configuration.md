...
| `marker_shape` | No | `"box"` | Trail-marker chip shape: `box` (rounded rect) or `pill`. The chip is variable-width to fit the ref text. A short ref (`"23"`, `"#"`) renders `pill` as a true circle; a longer ref (`"EAP-1"`) renders it as a rounded pill. |
