...
| `relation_names` | No | `{}` | Map of relation ID to display name; overrides the OSM `name` tag everywhere the route name appears (routes panel, on-map route labels, popups, search, alphabetical panel ordering). Useful when the OSM name is formally correct but unwieldy on a map (e.g. renaming "Pontiac Lake Recreation Area Mountain Bike Trail" to "Mountain Bike Trail"). Keys must be *leaf* route relation IDs — if a super-relation is listed in `relations:`, rename its child routes, not the parent (the build warns and lists the child IDs if you key the parent). Applied at build time post-cache: adding, changing, or removing an override takes effect on the next plain rebuild, no `--trails` refetch needed. Custom routes are unaffected (they set `name` inline). |
