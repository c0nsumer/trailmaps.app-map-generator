...
def collect_text_from_config(config):
    """Extract every string value from a (nested) config structure.

    Feeds the webfont coverage check: curator-authored config strings
    (brand name, welcome text, route names, ...) all surface in DOM
    chrome. Over-collects on purpose - a file path or internal key that
    never renders costs nothing to scan, and in practice those are
    ASCII, while missing a rendered string would silence the warning
    ...
    if isinstance(config, str):
        ...
        for v in config.values():
            chars |= collect_text_from_config(v)
...
def load_webfont_coverage(webfonts_src):
    """Union of codepoints covered by the bundled UI webfonts.

    Reads the committed .coverage.json sidecars (regenerated by
    tools/update_webfont_coverage.py whenever a .woff2 changes) rather
    than parsing the WOFF2 itself, so the build needs no font-parsing
    dependencies. Returns None when no sidecar exists, which callers
    treat as "coverage unknown - skip the check".
    """
    if not os.path.isdir(webfonts_src):
        return None
    ranges = []
    for fname in sorted(os.listdir(webfonts_src)):
        if not fname.endswith(".coverage.json"):
            continue
        try:
            with open(os.path.join(webfonts_src, fname), encoding="utf-8") as f:
                ranges.extend(json.load(f)["codepoint_ranges"])
        except (OSError, ValueError, KeyError, TypeError) as e:
            console.warn(f"Unreadable webfont coverage sidecar {fname}: {e}")
    if not ranges:
        return None
    covered = set()
...
def check_webfont_coverage(chars, webfonts_src):
    """Warn about characters the bundled UI webfont cannot render.

    DOM chrome text (popups, routes panel, marker chips, config-driven
    strings) renders in the self-hosted webfont; a character outside its
    subset silently falls back to the platform system font, re-introducing
    exactly the per-OS rendering drift the bundle exists to remove. This
    makes that fallback visible at build time. Warn-only by design: a
    stray character in one OSM name shouldn't block a deploy.

    Returns the set of uncovered characters (empty when fully covered,
    None when no coverage sidecar is available to check against).
    """
    covered = load_webfont_coverage(webfonts_src)
    if covered is None:
        console.info("No webfont coverage sidecar - skipping UI font coverage check")
        return None
    # Controls and whitespace never draw visible glyphs; spaces in
    # particular are in every fallback font at identical advance.
    missing = {c for c in chars if ord(c) not in covered and not c.isspace() and ord(c) >= 0x20}
    if missing:
        sample = ", ".join(f"'{c}' (U+{ord(c):04X})" for c in sorted(missing)[:10])
        more = f" (+{len(missing) - 10} more)" if len(missing) > 10 else ""
        console.warn(
            f"{len(missing)} character(s) in map text fall outside the bundled UI font "
            f"and will render in platform system fonts: {sample}{more}. "
            f"Consider vendoring a covering subset in assets/webfonts/ "
            f"(then run tools/update_webfont_coverage.py)."
        )
    return missing


def warn_uncovered_canvas_ranges(all_chars, needed_ranges, needed_faces, fonts_src):
    """Warn about needed glyph ranges that exist in NO shipped face.

    The trimmer adapts to whatever characters the data contains, but it
    can only ship ranges that exist under assets/fonts/ - a script with
    no vendored face (e.g. CJK) previously sailed through silently and
    rendered as tofu boxes on the map canvas at runtime.
    """
    available = set()
    for face in needed_faces:
        face_src = os.path.join(fonts_src, face)
        try:
            available.update(f for f in os.listdir(face_src) if f.endswith(".pbf"))
        except OSError:
            continue
    missing = sorted(
        (start, end) for start, end in needed_ranges if f"{start}-{end}.pbf" not in available
    )
    if not missing:
        return missing
    examples = []
    for start, end in missing[:5]:
        sample = "".join(c for c in sorted(all_chars) if start <= ord(c) <= end)[:3]
        examples.append(f"U+{start:04X}-{end:04X} ('{sample}')")
    more = f" (+{len(missing) - 5} more ranges)" if len(missing) > 5 else ""
    console.warn(
        f"{len(missing)} glyph range(s) needed by map text exist in no shipped font "
        f"face - those characters will render as boxes on the map canvas: "
        f"{', '.join(examples)}{more}. Add a covering face to assets/fonts/ and "
        f"SCRIPT_FONT_BLOCKS in font_trimmer.py."
    )
    return missing
